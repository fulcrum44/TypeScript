...
const Submarino_1 = require("./Submarino");
class Mapa {
    inicioMapa = null; // En una lista enlazada simple o doble, al menos hay que almacenar el primer elemento o el último. El mapa es una doble lista enlazada en dos dimensiones. La esquina superior-izquierda del tablero nos sirve como primer elemento del mapa-tablero
    numeroFilas;
    numeroColumnas;
    casillaSubmarino = null;
    constructor(size) {
        this.numeroFilas = size;
        this.numeroColumnas = size;
        this.generarMapa(Submarino_1.Submarino.posicionInicialAleatorio(size));
    }
    // La construcción del mapa la hacemos de arriba a abajo y de izquierda a derecha
    generarMapa(posicionSubmarino) {
        ...
        let inicioFila = null;
        let filaSuperior = null; // En la primera fila las casillas no tienen otra casilla por arriba, por lo tanto definimos que es null
        for (let i = 0; i < this.numeroFilas; i++) {
            let casillaActual = null;
            let columnaIzquierda = null; // La primera casilla no tiene otra casilla a si izquierda, por lo tanto definimos que es null
            for (let j = 0; j < this.numeroColumnas; j++) {
                casillaActual = new Casilla_1.Casilla(0);
                ...
                if (i == 0 && j == 0) {
                    ...
                    inicioFila = casillaActual; // Referencia a la primera casilla de la fila. Su uso es para luego definir el inicio de la fila anterior de la fila siguiente que generemos
    ...
    mostrarMapa() {
        let coordenadaFila = this.inicioMapa;
        while (coordenadaFila != null) {
            let coordenadaColumna = coordenadaFila;
